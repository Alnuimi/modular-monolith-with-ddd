...
) : ICommandHandler<InviteReviewerCommand, InviteReviewerResponse>
{
    ...
    private EmailMessage BuildEmailMessage(Article article, ReviewInvitation invitation, Editor editor)
    {
        const string InvitationEmail = 
            @"Dear Contributor, <br/>
                ...
                Please accept or reject the invitation will expire on {2}. <br/>
                If you don't have an account, please create one using the following URL: {3}
            ";

        var url = 
            _appUrlsOptions.Value.ReviewUIBaseUrl
                .AppendPathSegment($"articles/{invitation.ArticleId}/invitations/{invitation.Id}/{invitation.Token}");
