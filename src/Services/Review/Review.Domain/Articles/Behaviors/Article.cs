...
public partial class Article
{
    ...
    public ReviewInvitation InviteReviewer(Reviewer reviewer, IArticleAction action)
    {
        ...
        {
            throw new DomainException($"Reviewer {reviewer.FullName} does not have specialization in article's journal.");
        }
        return CreateInvitation(reviewer.UserId, reviewer.Email.Value, reviewer.FirstName, reviewer.LastName, action);
    }
    ...
    private ReviewInvitation CreateInvitation(int? userId, string email, string firstName, string lastName, IArticleAction action)
    {
        ...
        var invitation = new ReviewInvitation
        {
            ...
            ExpiresOn = DateTime.UtcNow.AddDays(7), // we can make this configurable later
