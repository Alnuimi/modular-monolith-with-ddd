...
namespace Submission.Application.Features.CreateArticle
{
    internal sealed class CreateArticleCommandHandler(Repository<Journal> _journalRepository) : IRequestHandler<CreateArticleCommand, IdResponse>
    {
        ...
        {
            var journal = await _journalRepository.FindByIdOrThrowAsync(command.JournalId); // todo - throw NotFoundException if journal not found
        ...
        {
            var author = await _journalRepository.Context.Authors.FirstOrDefaultAsync(x => x.UserId == command.CreatedById);
